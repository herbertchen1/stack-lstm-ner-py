...
struct ParserBuilder {
  ...
  LSTMBuilder ent_lstm_fwd; //entity
...
//整理action序列到map<int,string>
map<int,string> compute_ents(
    unsigned sent_len, 
...
pair<vector<unsigned>, Expression> log_prob_parser(ComputationGraph* hg,
  ...
  if (apply_dropout) {
    ...
    //buffer[i] = Relu(ib, w2l, w, [p2l, p])
    //end of init buffer[0~sent.size()]

    // dummy symbol to represent the empty buffer
    // init buffer/stack_lstm
    ...
    while(buffer.size() > 1 || stack.size()>1) {
      ...
      // Pick a single element/row/column/sub-tensor from an expression.
    ...
    assert(bufferi.size() == 1);
    Expression tot_neglogprob = -sum(log_probs);  //This performs an elementwise sum over all the expressions in xs
  ...
  if (conf.count("train")) {
    ...
    while(!requested_stop) {
      ...
      for (unsigned sii = 0; sii < status_every_i_iterations; ++sii) {
          //EACH ITER
          //LEARNING RATE DECAY & SHUFFLE CORPUS & UPDATE
